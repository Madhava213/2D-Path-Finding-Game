...
int closestNode(Vec2 point, Vec2[] nodePos, int numNodes, Vec2 oppositeEnd){
  int finalNodeIndex = 0;
  float distBetweenEnds = oppositeEnd.distanceTo(point);
  float finalDistance = nodePos[finalNodeIndex].distanceTo(point);
  for (int i = 1; i < numNodes; i++){
      float distFromPoint = nodePos[i].distanceTo(point);
      float distFromOpposite = nodePos[i].distanceTo(oppositeEnd);
      if(finalDistance > distFromPoint && distFromOpposite < distBetweenEnds){
        finalDistance = distFromPoint;
        finalNodeIndex = i; 
      }
  }
